...
class ClusterCommand extends DefaultMessage with Logger {
  ...
  private def stopClients(config:ClusterWeaver#Config, zk:ZooKeeperClient) {
    ...
    for {ci <- ClientInfoHelper.collectClientInfo(config, zk)
  ...
  def list(@option(prefix="--status", description="check status")
    ...
    for(zk <- f.defaultZkClient; actorSystem <- ActorService(localhost)){
      val  nodes = ClientInfoHelper.collectClientInfo(f.config, zk)
...
//  }
  ...
  def listServerStatus: Seq[(Node, String)] = {
    ...
    val s = for {
      ...
      ci <- ClientInfoHelper.collectClientInfo(f.config, zk).par
