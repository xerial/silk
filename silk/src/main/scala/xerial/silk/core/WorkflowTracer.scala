...
/**
 * WorkflowTracer finds dependencies between functions. If the target function is wrapped with SilkFlow that takes
 * a function as an argument (e.g., Map, Filter, etc.), WorkflowTracer build a dependency graph beginning from the function.
 *
 * In Scala, Function0, Function1, etc. are wrapped with an anonymous class, in which apply method is defined.
 *
 *
 *
 *
...
object WorkflowTracer extends Logger {
   ...
   * @return
   */
  private def methodFilter(name: String, argLen: Int): MethodRef => Boolean = {
    m: MethodRef =>
  ...
  private def resolveMethodCalledByFunctionRef[A](funcClass: Class[A]): Option[MethodRef] = {
    ...
    val mc = findMethodCall(funcClass, {
      m: MethodRef =>
